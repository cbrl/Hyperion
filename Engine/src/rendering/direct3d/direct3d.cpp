...
void Direct3D::Init() {
	...
	ThrowIfFailed(device->QueryInterface(__uuidof(IDXGIDevice), reinterpret_cast<void**>(dxgiDevice.GetAddressOf())),
	              "QueryInterface failed on dxgiDevice");

	ComPtr<IDXGIAdapter> dxgiAdapter;
	ThrowIfFailed(dxgiDevice->GetParent(__uuidof(IDXGIAdapter), reinterpret_cast<void**>(dxgiAdapter.GetAddressOf())),
...
void Direct3D::ReadRefreshRate(UINT& numerator, UINT& denominator) const {
	...
	adapterOut->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &modes, nullptr);

	std::vector<DXGI_MODE_DESC> displayModeList(modes);
	adapterOut->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM,
...
void Direct3D::ResizeBuffers(u32 win_width, u32 win_height) {
	...
	ThrowIfFailed(device->CreateRenderTargetView(backBuffer.Get(),
	                                             nullptr,
	...
	ThrowIfFailed(device->CreateTexture2D(&depthStencilDesc, nullptr, &depth_stencil_buffer),
