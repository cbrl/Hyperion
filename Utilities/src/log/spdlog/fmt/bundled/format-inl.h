...
FMT_FUNC void grisu2_prettify(const gen_digits_params &params,
  ...
  if (!params.fixed) {
    ...
    handler.insert(1, '.');
    exp += int_size - 1;
    if (int_size < params.num_digits)
      handler.append(params.num_digits - int_size, '0');
  ...
  if (int_size <= full_exp && full_exp <= exp_threshold) {
	...
	} else if (params.num_digits > int_size) {
      // Add trailing zeros.
      ptrdiff_t num_zeros = params.num_digits - int_size;
      handler.append(num_zeros, '0');
    }
  } else {
    // 1234e-6 -> 0.001234
